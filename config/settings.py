"""Загрузка настроек: .env (секреты, инфраструктура) + client_config.yaml (данные бизнеса).

Порядок применения:
  1. .env          — токены и URL (никогда не хардкодятся в код).
  2. client_config.yaml — всё, что относится к конкретному бизнесу.
Под нового клиента правится только client_config.yaml.
"""
...
def _load_config() -> dict:
    ...
    if not CONFIG_PATH.exists():
        raise FileNotFoundError(
            f"Не найден конфиг клиента: {CONFIG_PATH}. "
            "Создайте config/client_config.yaml по образцу из README."
        )
...
def get_settings() -> Settings:
    """Собирает итоговые настройки из .env и client_config.yaml.
    ...
    missing = [
        ...
        for name, value in {
            ...
            "business_name (client_config.yaml)": settings.business_name,
            "knowledge_base (client_config.yaml)": settings.knowledge_base,
            "llm.model (.env или client_config.yaml)": settings.llm.model,
        }.items()
        if not value
    ]
    if missing:
